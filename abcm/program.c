...
BYTE bootstrap[] = {
    0x06, 0x10, 0x82, 0xb8, 0x03, 0x81, 0x07, 0x10, 0x82, 0xb2, 0x03, 0x84, 0x0a, 0x84, 0x6b, 0x69,  // ···¸·····²····ki
    ...
    0x74, 0x73, 0x81, 0x0a, 0x86, 0x73, 0x63, 0x72, 0x69, 0x70, 0x74, 0x06, 0x10, 0x82, 0x7d, 0x03,  // ts···script···}·
    ...
    0x73, 0x65, 0x6c, 0x66, 0x07, 0xf1, 0x83, 0x0a, 0x84, 0x6b, 0x69, 0x6e, 0x64, 0x0a, 0x8a, 0x61,  // self·ñ···kind··a
...
BYTE actor_eval(sponsor_t * sponsor, actor_t * actor, DATA_PTR expression, DATA_PTR * result) {
    ...
    if (value_equiv(kind, k_expr_literal)) {
        ...
        *result = v_null;  // default value is `null`
        if (!object_get(actor->state, name, result)) {
        ...
        // FIXME: make sure `name` is a String...
        DATA_PTR dict;
        if (!property_eval(sponsor, actor, expression, s_in, &dict)) return false;  // evaluation failed!
        ...
        *result = v_null;  // default value is `null`
        ...
        // FIXME: make sure `name` is a String...
        DATA_PTR value;
        if (!property_eval(sponsor, actor, expression, s_value, &value)) return false;  // evaluation failed!
        DATA_PTR dict;
        if (!property_eval(sponsor, actor, expression, s_with, &dict)) return false;  // evaluation failed!
        // FIXME: make sure `dict` is an Object...
        *result = v_null;  // default value is `null`
        ...
        *result = o_;  // default to empty Object
        // FIXME: NEED TO GET THE MESSAGE FROM THE EVENT, NOT THE ACTOR!
    } else if (value_equiv(kind, k_actor_self)) {
        LOG_DEBUG("actor_eval: actor_self expression", (WORD)kind);
        // { "kind":"actor_self" }
        *result = actor->capability;
